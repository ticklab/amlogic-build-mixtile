...
//#define	AD85030_REG_RAM_CHECK

#ifdef CONFIG_HAS_EARLYSUSPEND
#include <linux/earlysuspend.h>
static void ad85030_early_suspend(struct early_suspend *h);
static void ad85030_late_resume(struct early_suspend *h);
#endif

#define AD85030_RATES (SNDRV_PCM_RATE_32000 | \
...
#define AD85030_FORMATS (SNDRV_PCM_FMTBIT_S16_LE | \
...
static const struct snd_kcontrol_new ad85030_snd_controls[] = {
...
static struct snd_soc_codec *ad85030_codec;

static int m_reg_tab[AD85030_REGISTER_COUNT][2] = {
...
struct ad85030_priv {
	...
	struct ad85030_platform_data *pdata;
#ifdef CONFIG_HAS_EARLYSUSPEND
...
static int ad85030_set_dai_sysclk(struct snd_soc_dai *codec_dai,
				  int clk_id, unsigned int freq, int dir)
{
	return 0;
}

static int ad85030_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
...
static int ad85030_hw_params(struct snd_pcm_substream *substream,
...
static int ad85030_set_bias_level(struct snd_soc_codec *codec,
...
static int ad85030_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	int ret;
	struct snd_soc_codec *codec = dai->codec;
	printk("%s---------------%d\n", __func__,__LINE__);
	
	ret=gpio_request(122,NULL);				// request amp SD pin control GPIO
	printk(" gpio 122 ret=%d\n",ret);
	gpio_direction_output(122, 1);		// pull high amp SD pin
	mdelay(20);
	//ad85030_init(codec);							// init amp again
	snd_soc_write(codec, 0x02, 0x00);   //--unmute amp
	
  return 0;
}

static void ad85030_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	int ret;
	struct snd_soc_codec *codec = dai->codec;
	printk("%s---------------%d\n", __func__,__LINE__);
	...
	ret=gpio_request(122,NULL);								// request amp SD pin control GPIO
	printk(" gpio 122 ret=%d\n",ret);
	gpio_direction_output(122, 0);						// pull low amp SD pin
	return;
}

static const struct snd_soc_dai_ops ad85030_dai_ops = {
	.startup = ad85030_startup,
	.shutdown = ad85030_shutdown,
	.hw_params = ad85030_hw_params,
	.set_sysclk = ad85030_set_dai_sysclk,
	.set_fmt = ad85030_set_dai_fmt,
};

static struct snd_soc_dai_driver ad85030_dai = {
	.name = "ad85030",   
	.playback = {
...
//дram
static int ad85030_set_eq_drc(struct snd_soc_codec *codec)
{
	u8 i;
	// ch1 ram
	for (i = 0; i < AD85030_RAM_TABLE_COUNT; i++) {
...
static int ad85030_reg_init(struct snd_soc_codec *codec)
{
	int i = 0;
	for (i = 0; i < AD85030_REGISTER_COUNT; i++) {            
...
#ifdef	AD85030_REG_RAM_CHECK
static int ad85030_reg_check(struct snd_soc_codec *codec)
{
	int i = 0;
	int reg_data = 0;
	...
	for (i = 0; i < AD85030_REGISTER_COUNT; i++) {
		...
		printk("ad85030_reg_init  write 0x%x = 0x%x\n", m_reg_tab[i][0], reg_data);
	};
	return 0;
}


static int ad85030_eqram1_check(struct snd_soc_codec *codec)
{
	int i = 0;
	...
	for (i = 0; i < AD85030_RAM_TABLE_COUNT; i++) {
		...
		printk("ad85030_set_eq_drc ram1  write 0x%x = 0x%x , 0x%x , 0x%x\n", m_ram1_tab[i][0], H8_data,M8_data,L8_data);
	};
	return 0;
}
#endif

static int ad85030_init(struct snd_soc_codec *codec)
{
	...
	// pull high SD pin if SD pin have controlled by GPIO
  ...
  // software reset amp , if no have hardware reset pin(48pin's ad85030)
	...
	// init AMP 
	dev_info(codec->dev, "ad85030_init!\n");

 	snd_soc_write(codec, 0x02, 0x7f);//--mute amp

	// write amp register
	ad85030_reg_init(codec);

  //The following line is mute again, in case ad85030_reg_init(codec) has unmute when writing the whole register.
	snd_soc_write(codec, 0x02, 0x7f);//--mute amp
  udelay(100);
  
	// write amp ram (eq and drc ... ) 
	ad85030_set_eq_drc(codec);
	udelay(100);
...
static int ad85030_probe(struct snd_soc_codec *codec)
{
   //int ret = 0;
   
  struct ad85030_priv *ad85030 = snd_soc_codec_get_drvdata(codec);
  printk("%s\n", __func__);
...
//	struct ad85030_priv *ad85030 = snd_soc_codec_get_drvdata(codec);
	...
	ad85030->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
	ad85030->early_suspend.suspend = ad85030_early_suspend;
	ad85030->early_suspend.resume = ad85030_late_resume;
	//ad85030->early_suspend.param = codec;
	register_early_suspend(&(ad85030->early_suspend));
#endif

	ad85030_codec = codec;
	
	//ad85030->codec = codec;
	codec->control_data = ad85030->control_data;
	ad85030_init(codec);

	return 0;
}

static int ad85030_remove(struct snd_soc_codec *codec)
{
#ifdef CONFIG_HAS_EARLYSUSPEND
	struct ad85030_priv *ad85030 = snd_soc_codec_get_drvdata(codec);

	unregister_early_suspend(&(ad85030->early_suspend));
#endif
	return 0;
}

static int ad85030_suspend(struct snd_soc_codec *codec)
{
	dev_info(codec->dev, "ad85030_suspend!\n");
...
static int ad85030_resume(struct snd_soc_codec *codec)
{
	dev_info(codec->dev, "ad85030_resume!\n");
...
static const struct snd_soc_dapm_widget ad85030_dapm_widgets[] = {
 ...
 */
static int ad85030_i2c_read(struct i2c_client *ad85030_client,
                u8 reg, u8 *value, int len)
{
    ...
    do {
        err = i2c_transfer(ad85030_client->adapter, msgs,
                            ARRAY_SIZE(msgs));
    ...
    if (err != ARRAY_SIZE(msgs)) {
        dev_err(&ad85030_client->dev, "read transfer error %d\n"
                , err);
        err = -EIO;
    } else {
        err = 0;
...
static unsigned int ad85030_read(struct snd_soc_codec *codec, unsigned int reg)
{
   // struct ad85030_priv *ad85030  = snd_soc_codec_get_drvdata(codec);
    u8 data;
    int len = 1;
    ...
    if (ad85030_i2c_read(codec->control_data, reg & 0xff, &data, len) == 0) {
        val = data;		//(buf[0] << 8 | buf[1]);
    }

    return val;
}
static int ad85030_i2c_write(struct i2c_client *ad85030_client,
                u8 *value, u8 len)
{
    int err;
    int tries = 0;
    ...
    if (err != ARRAY_SIZE(msgs)) {
        dev_err(&ad85030_client->dev, "write transfer error\n");
        err = -EIO;
    } else {
        err = 0;
    }
...
static int ad85030_write(struct snd_soc_codec *codec, unsigned int reg,
                unsigned int val)
{
    u8 buf[2] = {0, 0};
    int ret;
    ...
    ret = ad85030_i2c_write(codec->control_data, buf, ARRAY_SIZE(buf));
...
static const struct snd_soc_codec_driver soc_codec_dev_ad85030 = {
	.probe = ad85030_probe,
	.remove = ad85030_remove,
	.suspend = ad85030_suspend,
	.resume = ad85030_resume,
	.read =    ad85030_read,
  .write =   ad85030_write,
	.set_bias_level = ad85030_set_bias_level,
	...
	.component_driver = {
#endif
		.controls = ad85030_snd_controls,
		.num_controls = ARRAY_SIZE(ad85030_snd_controls),
		.dapm_widgets = ad85030_dapm_widgets,
		.num_dapm_widgets = ARRAY_SIZE(ad85030_dapm_widgets),
#if (KERNEL_VERSION(4, 0, 0) <= LINUX_VERSION_CODE)
	}
#endif
};

static int ad85030_i2c_probe(struct i2c_client *i2c,
			     const struct i2c_device_id *id)
{
	struct ad85030_priv *ad85030;
	struct ad85030_platform_data *pdata;
	enum of_gpio_flags flags;
	int ret;
  printk("%s---------------%d\n", __func__,__LINE__);
	ad85030 = devm_kzalloc(&i2c->dev, sizeof(struct ad85030_priv),
			       GFP_KERNEL);
	if (!ad85030)
		return -ENOMEM;
       /*       
	ad85030->regmap = devm_regmap_init_i2c(i2c, &ad85030_regmap);
	if (IS_ERR(ad85030->regmap)) {
		ret = PTR_ERR(ad85030->regmap);
		dev_err(&i2c->dev, "Failed to allocate register map: %d\n",
			ret);
		return ret;
	}*/
	i2c_set_clientdata(i2c, ad85030);

	pdata = devm_kzalloc(&i2c->dev, sizeof(struct ad85030_platform_data), GFP_KERNEL);
	if (!pdata) {
		pr_err("%s failed to kzalloc for ad85030 pdata\n", __func__);
		return -ENOMEM;
	}
	ad85030->pdata = pdata;
	
	ad85030->control_data = i2c;

	//ad85030_parse_dt(ad85030, i2c->dev.of_node);

	ret = snd_soc_register_codec(&i2c->dev, &soc_codec_dev_ad85030, &ad85030_dai, 1);
	if (ret != 0)
       {
...
static int ad85030_i2c_remove(struct i2c_client *client)
{
...
/*
void ad85030_i2c_shutdown(struct i2c_client *client)
{
	//struct snd_soc_codec *codec;
	//codec = ad85030_codec;
	struct ad85030_priv *ad85030 = snd_soc_codec_get_drvdata(ad85030_codec);
	struct ad85030_platform_data *pdata = ad85030->pdata;
...
static const struct i2c_device_id ad85030_i2c_id[] = {
	{ "ad85030", 0 },
	{},
};

MODULE_DEVICE_TABLE(i2c, ad85030_i2c_id);

static const struct of_device_id ad85030_of_id[] = {
	{ .compatible = "ESMT, ad85030"},
	{ },
};
//MODULE_DEVICE_TABLE(of, ad85030_of_id);

static struct i2c_driver ad85030_i2c_driver = {
	.driver = {
		.name = "ad85030",
		.of_match_table = ad85030_of_id,
		.owner = THIS_MODULE,
	},
	//.shutdown = ad85030_i2c_shutdown,
	.probe = ad85030_i2c_probe,
	.remove = ad85030_i2c_remove,
	.id_table = ad85030_i2c_id,
};

//module_i2c_driver(ad85030_i2c_driver);

static int __init ad85030_i2c_init(void)
{
	printk("%s\n", __func__);
	return i2c_add_driver(&ad85030_i2c_driver);
}

static void __exit ad85030_i2c_exit(void)
{
	i2c_del_driver(&ad85030_i2c_driver);
}

module_init(ad85030_i2c_init);
module_exit(ad85030_i2c_exit);

MODULE_DESCRIPTION("ASoC ad85030 driver");
MODULE_AUTHOR("ESMT");
MODULE_LICENSE("GPL");
